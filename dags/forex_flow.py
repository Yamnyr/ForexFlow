...
def forex_flow():
    ...
    def init_database():
        ...
        logging.info("Base de données et vues initialisées avec succès.")

    @task(retries=3, execution_timeout=timedelta(seconds=30))
    def extract_forex_data(ds=None, **kwargs):
        """Extrait les données de l'API Frankfurter et les stocke en brut."""
        base_curr = Variable.get("forex_base_currency", default_var="EUR")
        targets = Variable.get("forex_target_currencies", default_var="USD,GBP,JPY,CHF,CAD")
        
        url = f"{API_URL}/{ds}?from={base_curr}&to={targets}"
        
        logging.info(f"Interrogation de l'API: {url}")
    ...
    def validate_and_transform(extraction_result, **kwargs):
        """Valide, transforme et charge les données dans la table clean."""
        ...
        if not all(k in data for k in ['amount', 'base', 'date', 'rates']):
            ...
            rejection_reason = f"Nombre de devises insuffisant: {len(data.get('rates', {}))}"
        ...
        if abs((exec_date - api_date).days) > freshness_threshold:
            rejection_reason = f"Donnée trop ancienne (écart > {freshness_threshold} jours)"

        hook = PostgresHook(postgres_conn_id=POSTGRES_CONN_ID)
        
        if rejection_reason:
            logging.warning(f"Rejet de la donnée raw_id {raw_id}: {rejection_reason}")
            hook.run(
        ...
        for _, row in df.iterrows():
            ...
            """, (row['rate_date'], row['base_currency'], row['target_currency'], row['rate']))
    ...
    def detect_alerts(extraction_result):
        """Détecte les variations anormales par rapport au dernier taux connu."""
        data = extraction_result['data']
        ...
        for target, new_rate in data['rates'].items():
            sql = "SELECT rate FROM clean_forex WHERE base_currency = %s AND target_currency = %s AND rate_date < %s ORDER BY rate_date DESC LIMIT 1"
            ...
            if prev_rate_row:
                ...
                if variation > threshold:
                    hook.run(
                        "INSERT INTO alerts_forex (currency_pair, old_rate, new_rate, variation_pct) VALUES (%s, %s, %s, %s)",
    ...
    def log_pipeline_metrics(stats, alerts_count, **kwargs):
        """Journalise les résultats du run."""
        hook = PostgresHook(postgres_conn_id=POSTGRES_CONN_ID)
        hook.run("""
            INSERT INTO logs_forex (run_id, status, lines_received, lines_valid, lines_rejected, lines_inserted, execution_date)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
        """, parameters=(kwargs['run_id'], 'SUCCESS', stats['received'], stats['valid'], stats['rejected'], stats['inserted'], kwargs['logical_date']))
